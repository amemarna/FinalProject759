...
void readmatrix_f(FILE* fp, double* in, int nrows, int ncols)
{
...
void readmatrix_i(FILE* fp, int* in, int nrows, int ncols)
{
  for (int i=0;i<nrows;i++){
    for (int j=0;j<ncols;j++){
            double r=fscanf(fp,"%i",&in[i*ncols+j]);
...
void coordxtr(int* edof,double* coords, double* ex, double* ey, int nels)
{
...
{  
  ...
  FILE *fpe = fopen("edofdataf.inp","r");
  FILE *fpc = fopen("coordataf.inp","r");
  FILE *fph = fopen("hooke0.inp","r");
  FILE *fpg = fopen("gamma.inp","r");
  FILE *fpb = fopen("bcdataf.inp","r");
  FILE *fpl = fopen("bcldataf.inp","r");

  const char* fpew = "edof.out";
  
  int nels=1584;//528;//176;
  int neldofs=6;
  int nods=812;//284;//108;
  int ndofs = 1624;//568;//216;

  int* edofs  = (int*) malloc(nels*(neldofs+1)*sizeof(int));
  ...
  int* cstd  = (int*) malloc(24*sizeof(int));
  int* bcload  = (int*) malloc(11*sizeof(int));

  readmatrix_i(fpe,edofs,nels,neldofs+1);

  readmatrix_f(fpc,coords,nods,2);
  checkmat(coords,nods,2);
  readmatrix_f(fph,De,6,6);

  readmatrix_f(fpg,gamma,9,6);

  readmatrix_i(fpb,cstd,1,24);
  readmatrix_i(fpl,bcload,1,11);
  
  double th    = 0.001;   
  
  
//% Analysis parameters
  double NrOfTsteps = 1000.0;//100.0; // % total number of timesteps
...
double* du = (double*) malloc(ndofs*sizeof(double));
 ...
 int nbc = 24;//24;
      ...
      int* cstd_ck =(int*) malloc(nbc*sizeof(int));
      int nfr0 = ndofs-nbc;
 ...
 for (int Tstep=0;Tstep<NrOfTsteps;Tstep++){
  ...
  double* Kg = (double*) malloc(ndofs*ndofs*sizeof(double));          
	...
	free(t_sig3);free(t_sig);free(Deps3);free(Ce3);free(Se);
	...
	free(Ap);
      ...
      if(my_rank==0)
	printf("cg_iter= %d cg_err= %lf time2conv= %lf \n",cgit,err_cg,tcg1-tcg0);
