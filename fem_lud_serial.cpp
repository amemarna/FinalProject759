...
//% Material parameters for elastic plastic behavior
...
double* du = (double*) malloc(ndofs*sizeof(double));
 ...
 for (int Tstep=0;Tstep<NrOfTsteps;Tstep++){
  ...
  double* Kg = (double*) malloc(ndofs*ndofs*sizeof(double));          
	...
	break;}
      ...
      tlu1 = clock() - tlu0;
      printf("Tstep= %d; iter= %d ;err= %lf ; LU_time = %f \n",Tstep,iter,err,((float)tlu1)/CLOCKS_PER_SEC);
 ...
 free(fh);
 //printf("Step= %lf \n disp= %lf \n force= %lf \n total_time= %f \n",Tstepc,dispcontrolled,FReaction[99],((float)t1)/CLOCKS_PER_SEC);
 printf("TStep= %lf \n disp= %lf \n  total_time= %f \n",Tstepc,dispcontrolled,((float)t1)/CLOCKS_PER_SEC);
